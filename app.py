...
def get_posts():
    ...
    try:
        ...
        for post in posts:
            ...
            post["count_jempol"] = db.likes.count_documents(
                {"post_id": post["_id"], "type": "jempol"}
            )
            post["jempol_by_me"] = bool(
                db.likes.find_one(
                    {"post_id": post["_id"], "type": "jempol", "username": payload["id"]}
                )
            )
        return jsonify(
            {
...
def update_like():
    ...
    try:
        ...
        return jsonify({"result": "success", "msg": "updated", "count_heart": count_heart})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
...
def update_star():
    ...
    try:
        ...
        if action_receive == "like":
            ...
            db.likes.delete_one(doc)
        count_star = db.likes.count_documents(
            {"post_id": post_id_receive, "type": type_receive}
        )
        return jsonify({"result": "success", "msg": "updated", "count_star": count_star})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))

@app.route("/update_jempol", methods=["POST"])
def update_jempol():
    token_receive = request.cookies.get("mytoken")
    try:
        payload = jwt.decode(token_receive, SECRET_KEY, algorithms=["HS256"])
        ...
        if action_receive == "like":
            ...
            db.likes.delete_one(doc)
        count_jempol = db.likes.count_documents(
            {"post_id": post_id_receive, "type": type_receive}
        )
        return jsonify({"result": "success", "msg": "updated", "count_jempol": count_jempol})
    except (jwt.ExpiredSignatureError, jwt.exceptions.DecodeError):
        return redirect(url_for("home"))
