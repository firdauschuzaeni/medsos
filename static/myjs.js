    ...
    success: function (response) {
      if (response["result"] === "success") {
        ...
        for (let i = 0; i < posts.length; i++) {
          ...
          let class_jempol = post["jempol_by_me"] ? "fa-thumbs-up" : "fa-thumbs-o-up";
          let html_temp = `<div class="box" id="${post["_id"]}">
            ...
            post["username"]
                                                ...
                                                <nav class="level is-mobile">
                                                    ...
                                                    <a class="level-item is-sparta" aria-label="jempol" onclick="toggle_jempol('${post["_id"]}', 'jempol')">
                                                      <span class="icon is-small"><i class="fa ${class_jempol}"
                                                            aria-hidden="true"></i></span>&nbsp;<span class="like-num">${num2str(post["count_jempol"])}</span>
                                                    </a>
                                                    </div>
...
function toggle_jempol(post_id, type) {
  console.log(post_id, type);
  let $a_like = $(`#${post_id} a[aria-label='jempol']`);
  let $i_like = $a_like.find("i");
  if ($i_like.hasClass("fa-thumbs-up")) {
    $.ajax({
      ...
      success: function (response) {
        ...
        $a_like.find("span.like-num").text(num2str(response["count_jempol"]));
      },
    });
    } else {
    $.ajax({
